...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            email.send()
            registered = True
            return render(request,'login_register/register_done.html',{'message':'Thank you for register we will redirect you in 5 seconds or you can click this link','link':'login_register:login'})
        else:
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        user.is_active = True
        user.save()
        return render(request,'login_register/register_done.html',{'message':'Thank you for your email confirmation. Now you can login your account.','link':'login_register:login'})
    else:
